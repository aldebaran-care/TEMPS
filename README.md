...
2. Submit the job — `train.sh` exports `HF_HOME=$PWD/.hf_cache`, `TRANSFORMERS_OFFLINE=1`, `HF_HUB_OFFLINE=1`, so the compute node reads from the cache and never touches the network:
   ```bash
   sbatch train.sh
   ```
3. After the job, `train.sh` packages the TensorBoard event dir + metrics JSON + `log.csv` into `logs/<jobid>_<run>.tar.gz`. Pull it back and view locally:
   ```bash
