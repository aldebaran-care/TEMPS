        ...
        for benchmark in "${BENCHMARKS[@]}"; do
            ...
            for num_neg in "${NUM_NEGATIVE_SAMPLES[@]}"; do
                ...
                for external_model in "${EXTERNAL_MODELS[@]}"; do
                    ...
                    python3 evaluate.py --model_name=all-minilm-l6-v2-full --external_model_name=$external_model --model_path=output/trained_models/model_sentence-transformers_all-MiniLM-L6-v2_2025-07-25_23-47-04.pth --batch_size=128 --max_seq_len=512 --benchmark=$benchmark --eval_id=1 --top_k=$top_k --metric=all --alpha=$alpha --num_negative_samples=$num_neg | grep "^{'top':"
                done
            done
        done
    done
