...
from temporal_embeddings.data_utils.utils.dates.date_utils import get_week_dates, get_month_dates, get_year_dates

def offset_to_date(annotation: str, reference_date: str) -> List[str]:
    date_obj: datetime = datetime.strptime(reference_date, "%Y-%m-%d")
    ...
    if annotation_type == "d":
        ...
        if offset < 0:
            ...
            return [new_date.strftime("%Y-%m-%d"), reference_date]
    
        else:
            new_date = date_obj + timedelta(days=offset)
    
            return [reference_date, new_date.strftime("%Y-%m-%d")]
        ...
        return get_week_dates(reference_date, 1 - offset, num_weeks=offset)
    
    elif annotation_type == "thism":
        offset = extract_integers(annotation)[0]
    
        return get_month_dates(reference_date, 1 - offset, num_months=offset)
    
    elif annotation_type == "thisy":
        offset = extract_integers(annotation)[0]
    
        return get_year_dates(reference_date, 1 - offset, num_years=offset)
        ...
        if offset == 1:
            ...
            return sorted([new_date.strftime("%Y-%m-%d"), reference_date])
        ...
        return get_week_dates(reference_date, offset, abs(offset))
        ...
        return get_month_dates(reference_date, offset, abs(offset))
        ...
        return get_year_dates(reference_date, offset, abs(offset))
    
    raise ValueError(f"Cannot convert offset to date: {annotation}")