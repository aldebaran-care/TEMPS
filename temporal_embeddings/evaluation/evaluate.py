...
def evaluate_model(model_name: str, external_model_name: str, model_path: Path, batch_size: int, max_seq_len: int, benchmark: str, eval_id: int, top_k: int, metric: str, skip: bool = False, use_ranking: bool = False, a: float = 0.5, use_all_paragraphs: bool = False) -> None:
    dataset_file_path: Path = get_dataset_file_path(benchmark)

    if model_name in ["temporal_bert", "all-minilm-l6-v2"]:
        evaluate_temporal_bert(model_name, model_path, batch_size, max_seq_len, dataset_file_path, eval_id, top_k, metric, skip, use_all_paragraphs)
    
    elif model_name in ["temporal_bert_full", "all-minilm-l6-v2-full"]:
        evaluate_temporal_bert_full(model_name, external_model_name, model_path, batch_size, max_seq_len, dataset_file_path, eval_id, top_k, metric, skip, use_ranking, a, use_all_paragraphs)
        ...
        evaluate_sentence_transformer(model_name, max_seq_len, dataset_file_path, eval_id, top_k, metric, skip, use_all_paragraphs)