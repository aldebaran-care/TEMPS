...
def compute_metrics_ranks(
    first_list: List[List[int]],  # list of relevant indices per query
    second_list: List[List[int]],  # ranks
    top_k: int,
    metric: Literal['top', 'mrr', 'ndcg', 'precision', 'recall', 'f1', 'all']
) -> Dict[str, float]:
    if metric == 'all':
        metrics = ['top', 'mrr', 'ndcg', 'precision', 'recall', 'f1']
        results = {m: compute_single_metric_ranks(first_list, second_list, top_k, m) for m in metrics}
        return results
    else:
        return {metric: compute_single_metric_ranks(first_list, second_list, top_k, metric)}


def compute_single_metric_ranks(first_list: List[List[int]], second_list: List[List[int]], top_k: int, metric: Literal['top', 'mrr', 'ndcg', 'precision', 'recall', 'f1']) -> float:
    scores = []
    
    for gt_indices, ranks in zip(first_list, second_list):
        top_k_indices = ranks[:top_k]
        relevant_set = set(gt_indices)
